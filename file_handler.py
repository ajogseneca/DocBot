...
def handle_output(args, toml_dict, models_to_use, api_key, token):
    ...
    if output_path:
        try:
            ...
            print(f"An error occurred while writing to {output_path}: {str(e)}", file=sys.stderr)
            sys.exit(1)
    else:
...
def process_files(files, output_file, models_to_use, api_key, token):
    ...
    for file in files:
        try:
            ...
            if output_file:
                generate_readme(file, output_file=output_file, models=models_to_use, api_key=api_key, token=token)
            else:
                generate_readme(file, models=models_to_use, api_key=api_key, token=token)
        except Exception as e:
            print(f"An error occurred while processing {file}: {str(e)}", file=sys.stderr)
